...
matrix = get_distance_matrix(gene_trees,"rf")
link = get_linkage(matrix,"ward")
clustering = cluster_linkage(link,nclasses)
print clustering
showplot(matrix,clustering,link,names)
...
for key in assignments:
    print "Cluster {0}\nSize: {1}".format(key,len(assignments[key]))
    trees = []
...
print "\n\nOf {0} clusters, {1} are distinct".format(len (cl_matrix), groups)
cluster_link = get_linkage(cl_matrix,"ward")
cluster_clustering = cluster_linkage(link,0.25,criterion="distance")
print cluster_clustering
showplot(cl_matrix,cluster_clustering,cluster_link,cluster_names)
