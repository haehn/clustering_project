...
for fasta in fasta_files:
    ...
    if program == 'raxml':                                      # Refactor this code into function
        ...
        if write_genetrees:                                       # check for pre-calculated tree result
            ...
            except IOError: # print "Running RAxML on {0}".format(phylip)
                tree = run_raxml( model, alignment, name, nthreads=8 )
                tree.write_to_file( "{0}/{1}.tree".format(OUT_DIR,tree.name) )
            except AssertionError: pass# print "This tree was generated by", tree.program
        else: # print "Running RAxML on {0}".format(phylip)
            tree = run_raxml( model, alignment, name, nthreads=8 )
        ...
        else: # print "Running PhyML on {0}".format(phylip)
            tree = run_phyml( model, alignment, name, datatype)
        gene_trees.append( tree )
...
for key in assignments:
    ...
    if tr_tree: pass# print "Cluster vs True RF (topology) comparison:", min(trcomps), trcomps
...
print "{0}\t{1}".format(groups, final_score)
if len(cl_matrix) > 1:    
