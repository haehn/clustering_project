...
for i in range(C):
    #Write parameters
    write_ALF_parameters(simulation_name='class{0}'.format(i+1), experiment_directory=TEMP_DIR, simulation_directory='', number_of_genes=G[i], min_gene_length=130, number_of_species=S, mutation_rate=R[i], indels=True, gtr=gtr, output_filename='{0}/class{1}-params.drw'.format(OUT_DIR,i+1))   
