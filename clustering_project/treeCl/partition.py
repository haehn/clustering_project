...
class Partition(object):
    ...
    def __init__(self, partition_vector):
        """ self.partitions uses a compound key to retrieve partitions key =
        tuple of (distance_metric, linkage_method, num_classes) """
        ...
        for i in range(l1):
            ...
            for j in range(l2):
                ...
                if intersect == 0:
                    ...
                    mut_inf += intersect / total * log(total * intersect
                            / (len(m1[i]) * len(m2[j])), 2)
