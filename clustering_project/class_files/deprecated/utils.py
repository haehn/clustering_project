...
def equations(s,bl1,bl2,dec=""): #create right and left equations for all splits
    ...
    if not dec: splits=bl1.keys()
    else: splits=[i for i in bl1.keys() if i.startswith("%s/"%dec)]
...
def index(plist,dim):
    ...
    def index2(plist,dim):
        ...
        return s+index2(map(lambda x,z=1+min:x-z,plist),dim-min-1) #reduce plist by min+1
...
def splits_for_tree(root,term): #Creates split-list,bl-list, boolean poly (True if multifurcations or zero branch lengths), n (number of spp)
    ...
    for node in root.traverse():
        ...
        if splits.has_key(s):splits[s]+=node.branch #root splits one branch
        else:splits[s]=node.branch
    
    split_list=splits.keys()
    split_list.sort()
