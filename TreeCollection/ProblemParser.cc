...
namespace ProblemParser {
   ...
   PhyTree *parse_tree(istream &tree) throw (ParseException) {
      PhyTree *ptree = newick_parser::parse_newick(&tree);
      return ptree;
   }
   ...
   PhyTree *parse_tree(const string &tree) throw (ParseException) {
      stringstream stree(tree);
