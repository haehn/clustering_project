...
#define aDIST(k,i,j)              (aDistVar[k](aMap(k,i)-1,aMap(k,j)-1))
#define aVAR(k,i,j)               (dinv(aDistVar[k](aMap(k,j)-1,aMap(k,i)-1)))
...
void MinSquareTreeCollection::SolveRestricted( const DblMatrix &Z, const DblVector &B, DblVector &L, int n, int restr )
{
   ...
   for( i=0; i<n; i++ ) {
      if( (restr & (1<<i)) != 0 ) {
         ...
         ZZ(i,i) = 16*maxcol; BB[i] = MinLen*16*maxcol;
      }
...
restart:
      ...
      eps = 50*DBL_EPSILON*max;
      for( i=0; i<5; i++ ) if( L[i] < MinLen-eps ) {
         assert( (restr & (1<<i)) == 0 );
         restr |= 1<<i;
         goto restart;
      }
   ...
   /* requires inc[][] vector */
{
   ...
   for(int j=0; j<3; j++ ) {
      ...
      if( e1 != ExcludedEdge ) {
         if (EdgeC[e1].From==from) {
            dest = EdgeC[e1].To;     orig = EdgeC[e1].From;
         }
         else {
            dest = EdgeC[e1].From;   orig = EdgeC[e1].To;
         }
...
void MinSquareTreeCollection::getSons(int e0, int n, int* e1, int* e2)
{
   int k;
   k = n-ne;
   if(inc(k,0) == e0) { *e1 = inc(k,1); *e2 = inc(k,2); }
   else if(inc(k,1) == e0) { *e1 = inc(k,0); *e2 = inc(k,2); }
   else                     { *e1 = inc(k,0); *e2 = inc(k,1); }
   return;
}
...
void MinSquareTreeCollection::LabelNonExistEdges()
{
   ...
   Fix001Case(e0, e0, n);
   for(int k=0; k<NT; k++) if(EdgeC[e0].len[k] == -DBL_MAX)
      ...
      int* ix, int* a2, int* ee2, int* ia2, int* ex2)
{
   ...
   /* Leaf */
   if(n_next < ne) {
      (*ia)++; at[(*ia)-1] = n_next; et[(*ia)-1] = e0;
      /* the following is to track which at[.] are cherries */
      if(*ix == 2 && (*ia2) < 2) {
         (*ia2)++; a2[(*ia2)-1] = n_next; ee2[(*ia2)-1] = e0;
      }
      return;
   }
      ...
      double mbc,double *T)
{
   ...
   ab = mab/wab;  ac = mac/wac; bc = mbc/wbc;
   T[0] = (ab+ac-bc)/2;   T[1] = (ab-ac+bc)/2;   T[2] = (-ab+ac+bc)/2;
         ...
         for(int sj=si+1; sj<ne; sj++) {
            if(ShortestLabel[si] != ShortestLabel[sj]) {
               if(!isNA(k,sj)) {
                  ...
                  if(v==0) { Wei = 1.0e-10;  dd = 300.0; }
                  else { Wei = v; dd = aDIST(k,si,sj); }
                  W[i] += Wei;
...
void MinSquareTreeCollection::GotoLeaf(int i, int e0, int n_papa, int* eL, int *L)
{
   ...
   if(n_papa < ne) { (*eL) = e0; (*L) = n_papa; return; }
...
void MinSquareTreeCollection::FitEdge(int i, int e0, int allEdges)
{
   ...
   if(qOk == 0) {
      ...
      A = B = -1;
      C = a2[1-1]; D = a2[2-1]; eC = ee2[1-1]; eD = ee2[2-1];
      for(int j=0; j<4; j++) {
...
#                           */
   ...
   for( e3=2*ne-4; e3 >= 0; e3-- )
      if( EdgeC[e3].From >= ne && EdgeC[e3].To >= ne ) {
         ...
         if( inc(ix,0) == e3 ) { e1 = inc(ix,1);  e2 = inc(ix,2); }
         else if( inc(ix,1) == e3 ) { e1 = inc(ix,0);  e2 = inc(ix,2); }
         else                        { e1 = inc(ix,0);  e2 = inc(ix,1); }

         if( inc(iy,0) == e3 ) { e4 = inc(iy,1);  e5 = inc(iy,2); }
         else if( inc(iy,1) == e3 ) { e4 = inc(iy,0);  e5 = inc(iy,2); }
         else                        { e4 = inc(iy,0);  e5 = inc(iy,1); }
            ...
            for(int si=0; si<ne; si++ ) {
               ...
               for(int sj=si+1; sj<ne; sj++ ) {
                  if( ShortestLabel[si] != ShortestLabel[sj] ) {
                     ...
                     double v = aVAR(k,si,sj);
                     if( v==0 ) {
                        Wei = 1.0e-10;  dd = 300.0;
                     }
                     else {
                        Wei = v;        dd = aDIST(k,si,sj);
                     }
                     W(k,i) += Wei;
         ...
         if (!DoNotModify) {
            ...
            if( ReallyLessCollection(t2,t1) ) {
               ...
               EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
               EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vc;
               EdgeC[e4].From = iy+ne;  EdgeC[e4].To = vb;
               EdgeC[e5].From = iy+ne;  EdgeC[e5].To = vd;
               IncidencesC();
#ifdef DEBUG
            ...
            if( ReallyLessCollection(t3,t1) ) {
               ...
               EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
               EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vd;
               EdgeC[e4].From = iy+ne;  EdgeC[e4].To = vc;
               EdgeC[e5].From = iy+ne;  EdgeC[e5].To = vb;
               IncidencesC();
#ifdef DEBUG
               tmp = DistanceFitCollection();
      ...
      DblVector &L, int Skip)
{
   ...
   for( it=0; it<36; it++ ) {
      ...
      for( i=0; i<7; i++ ) {
         if( L[i] < MinLen-eps ) {
            ...
            Z2(i,i) = 10*maxcol; B2[i] = MinLen*10*maxcol;
            break;
   ...
   for( i=0; i<7; i++ ) if( L[i] < MinLen-eps ) {
      ...
      int ix, int iw, int iy, int e3, int e5)
{
   ...
   EdgeC[e6].From = iy+ne;
   EdgeC[e7].From = iy+ne;
   EdgeC[e3].From = ix+ne;  EdgeC[e3].To = iw+ne;
   EdgeC[e5].From = iy+ne;  EdgeC[e5].To = iw+ne;
   inc(ix,0) = e1;  inc(ix,1) = e2;  inc(ix,2) = e3;
   inc(iw,0) = e3;  inc(iw,1) = e4;  inc(iw,2) = e5;
   inc(iy,0) = e5;  inc(iy,1) = e6;  inc(iy,2) = e7;
}
      ...
      int iy,int iw, DblMatrix &L, IntVector &Skip, DblMatrix &M, DblMatrix &W, int *ptrnswap)
{
   ...
   if( inc(iy,0)==e5 ) { e6 = inc(iy,1);  e7 = inc(iy,2); }
   else if( inc(iy,1)==e5 ) { e6 = inc(iy,0);  e7 = inc(iy,2); }
   else                      { e6 = inc(iy,0);  e7 = inc(iy,1); }
...
/* requires inc[][] vector */
{
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vd;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vc;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vb;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = ve;
      NewTopology = 1;
#ifdef DEBUG
      IncidencesC();
      tmp = DistanceFitCollection();
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = ve;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vc;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vd;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = vb;
      NewTopology = 2;
#ifdef DEBUG
      IncidencesC();
      tmp = DistanceFitCollection();
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = vc;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vb;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = va;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vd;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = ve;
      NewTopology = 3;
#ifdef DEBUG
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vc;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vb;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vd;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = ve;
      NewTopology = 6;
#ifdef DEBUG
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vd;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vb;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vc;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = ve;
      NewTopology = 7;
#ifdef DEBUG
      IncidencesC();
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vb;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vd;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = vc;
      NewTopology = 8;
#ifdef DEBUG
      IncidencesC();
      tmp = DistanceFitCollection();
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vb;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = vd;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vc;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = ve;
      NewTopology = 9;
#ifdef DEBUG
   ...
   if( ReallyLessCollection(t2,t1) ) {
      ...
      EdgeC[e1].From = ix+ne;  EdgeC[e1].To = va;
      EdgeC[e2].From = ix+ne;  EdgeC[e2].To = vb;
      EdgeC[e4].From = iw+ne;  EdgeC[e4].To = ve;
      EdgeC[e6].From = iy+ne;  EdgeC[e6].To = vd;
      EdgeC[e7].From = iy+ne;  EdgeC[e7].To = vc;
      NewTopology = 12;
#ifdef DEBUG
...
#*************************************/
   ...
   for(int iter=0; iter<5; iter++ ) {
      ...
      for(int iw=ne-3; iw >= 0; iw-- ) if( DoEdge[iw] ) {
         DoEdge[iw] = 0;
         e3 = inc(iw,0);
         e4 = inc(iw,1);
         e5 = inc(iw,2);
...
int MinSquareTreeCollection::MapTree2InternalC( const PhyTree &t ) throw (ParameterException)
{
   ...
   EdgeC[e1].From = in1;  EdgeC[e1].To = in3+ne;
   ...
   EdgeC[e2].From = in2;  EdgeC[e2].To = in3+ne;
...
PhyTree* MinSquareTreeCollection::MST_TreeCR( int node, int edgefrom )
{
   int e1, e2;
   PhyTree *tree;

   if( node < ne )
      return( new PhyTree(Labels.size()==0 ? to_str(node+1) : Labels[node]) );

   if( inc(node-ne,0)==edgefrom )      { e1 = inc(node-ne,1);  e2 = inc(node-ne,2); }
   else if( inc(node-ne,1)==edgefrom ) { e1 = inc(node-ne,0);  e2 = inc(node-ne,2); }
   else                                { e1 = inc(node-ne,0);  e2 = inc(node-ne,1); }
...
PhyTree* MinSquareTreeCollection::getPhyTree()
{
   int e1, n[2], bestE=0;
   double p[2], bestDiff;
   ...
   for( e1=0; e1<2*ne-3; e1++ ) {
      ...
      if (abs(p[0]/n[0]-p[1]/n[1]) < bestDiff) {
         bestE = e1; bestDiff = abs(p[0]/n[0]-p[1]/n[1]);
      }
...
void MinSquareTreeCollection::compute(bool KeepTopology) throw (RuntimeException)
{
   int nswap, iter=6, i;
   double d1, d2;
   IntVector Skip;
   ...
   if (KeepTopology) {
      ...
      i = iter;
      while( i-- > 0 && ReallyLessGrob(d2,d1) ) {
         FitLabeledEdgesC(1);
         d1 = d2;
         d2 = DistanceFitCollection();
         printf("after fitting edges with FitLabeledEdgesC, fit %.12g (d1=%.12g)\n", d2,d1);
      }
      IncidencesC();
   }
   else {
      ...
      d2 = DistanceFitCollection();
      printf("after fitting edges with FitLabeledEdgesC, fit %.12g\n", d2);

      while( i-- > 0) {
         nswap = FiveOptimCollection(0,L,Skip,M,W);
         IncidencesC();
         d2 = DistanceFitCollection();
         printf("after FiveOptim, dst fit %.8g (%d swaps)\n",d2,nswap);

         /*  nswap = FourOptimCollection(0,L,Skip,M,W);
             IncidencesC();
             d2 = DistanceFitCollection();
#ifdef DEBUG
printf("after FourOptim, dst fit %.8g (%d swaps)\n",d2,nswap);
#endif
*/
         IncidencesC();
         FitLabeledEdgesC(1);
         ...
         d2 = DistanceFitCollection();
         printf("after fitting edges with FitLabeledEdgesC, %.8g\n", d2);

         if (nswap > 0)
            i = iter;

         if( ReallyLessGrob(d2,d1) ) { d1 = d2;  continue; }
         break;
      }
   }
   if (i <= 0)
...
string MinSquareTreeCollection::getTree()
{
   PhyTree *tree = getPhyTree();
   string tree_str = tree->formatNewick();
   delete tree;
   return tree_str;
}
