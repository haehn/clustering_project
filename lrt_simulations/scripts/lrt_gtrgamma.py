...
print 'getting trees...'
col.put_trees_parallel(program='phyml', model='GTR', datatype='nt',
                       ncat=4, tmpdir=tmpdir)

print 'getting partitions...'
col.put_partitions(metrics=['sym'], linkages=['ward'],
                   nclasses=range(1, 21))
col.put_clusters()
col.put_cluster_trees_parallel(program='phyml', model='GTR',
                               datatype='nt', ncat=4, tmpdir=tmpdir)

cPickle.dump(col, file('yeast_gtrgamma.pickle', 'w'))
